...
describe.skipIf(!testDatabaseUrl)('API integration', () => {
  ...
  test('POST /api/sneakers/from-catalog creates a sneaker from a catalog product', async () => {
    ...
    const created = (await response.json()) as {
      sneaker: {
        ...
        primaryImage: Array<{ id: string; url: string }> | null;
    ...
    expect(created.sneaker.primaryImage).toEqual({
  ...
  test('POST /api/sneakers/custom creates a sneaker from custom input', async () => {
    ...
    const input: CreateSneakerInput = {
      ...
      condition: 'deadstock',
      nickname: 'Big Bubble',
      primaryImage: 'https://images.example.com/air-force-1.png',
    };
  ...
  test('PATCH /api/sneakers/:id updates primary image for manual sneakers', async () => {
    ...
    const createResponse = await app.request('/api/sneakers/custom', {
      ...
      body: JSON.stringify({
        ...
        size: 10,
        condition: 'deadstock',
        primaryImage: 'https://images.example.com/original.png',
      }),
    });
    ...
    const updateResponse = await app.request(`/api/sneakers/${created.sneaker.id}`, {
      ...
      headers: { 'Content-Type': 'application/json', Cookie: cookie },
      body: JSON.stringify({
        primaryImage: 'https://images.example.com/updated.png',
      }),
    });
    ...
    expect(updated.sneaker.primaryImage).toEqual({
      id: expect.any(String),
  ...
  test('PATCH /api/sneakers/:id rejects catalog-linked image changes', async () => {
    ...
    expect(updateResponse.status).toBe(400);
    await expect(updateResponse.json()).resolves.toEqual({
      error: 'Cannot update primaryImage for catalog-linked sneakers',
    });
  });
